...
<b>Notes:</b>
Each item has only two possible states (taken or not taken), corresponding to binary 0 and 1.
This is an example of the <b>bounded knapsack problem</b>, where in comparison to the [[Integer Knapsack|unbounded case]], we can only take each item once.
...
> 	- OPT selects <u>best of</u> $\{1, 2, \ldots, k-1 \}$ to fill this new capacity.
...
> 	- OPT selects <u>best of</u> $\{1, 2, \ldots, k-1 \}$ to fill all $i$ units of weight.
> 	- The maximum value in this case is $\operatorname{opt}(i-w_k,k-1) + v_k$.
> 
> First check whether $w_k > i$. In this case the item doesn't fit so we are forced to not choose it (case 2). Otherwise, if it fits, we try both options of taking item $k$ or not taking item $k$, then use the <u>best of the two choices</u> (max of case 1 and 2). 
>
...
> 
> - $\operatorname{opt}(i, k)$ be the <b>maximum value</b> that can be achieved using up to $i$ units of weight <u>and</u> using only items among the first $k$ items, 
> - $\operatorname{m}(i, k)$ be the <b>(largest) index of an item</b> in such an collection.
> 
> <b>Recurrence:</b>
> 
...
> 
> Solve subproblems in increasing order of $k$  (and any order of $i$).
> 
...
> <b>Final answer:</b> 
> 
> $\text{opt}(C, n)$ which gives the maximum total value at capacity $C$ and using all $n$ items.
> 
> <b>Time complexity:</b>
> 
> There are $O(nC)$ subproblems, each taking $O(1)$ time to compute, for a total running time of **$O(nC)$** — again **pseudo-polynomial**.

## Explanation

> [!idea] The optimal substructure
> The optimal solution on items $\{ 1 \dots k \}$ with capacity $i$ depends on the optimal solutions to smaller problems on the set of items that does not include $k$ i.e., $\{ 1 \dots k-1 \}$. 

Each subproblem (state) depends on two variables: the remaining capacity of the knapsack, and one fewer item. Combining these two dependencies, we define our subproblems as:

> [!function] The set of subproblems
> For each $0 \leq i \leq C$ , let
>  - $\text{opt}(i, k)$ be the maximum total value the knapsack can carry with capacity $i$ and considering only the first $k$ items.
...
From this we can derive the recurrence:

> [!function] Two Parameters, Constant Recurrence
...
- These set of subproblems (and their recurrence) worked for the [[Integer Knapsack|Unbounded knapsack]] because we were allowed **unlimited copies** of each item, meaning we do not need to consider which input items are allowed for each subproblem because they all are. 
...
> [!function] The optimal substructure
...
The idea is if an optimal solution to the problem with capacity C includes an item k, then removing that item gives an optimal solution to the problem with a <u>smaller capacity C–wₖ </u> on <u>the set of items not including k</u>.
...
From our optimal substructure, we observe that the subproblems consider a smaller capacity AND one fewer item. 

Therefore, each subproblem tracks:
...
> [!function] The subproblem definition
> For each $0 \leq i \leq C$, let $\text{opt}(i, k)$ be the maximum total value the knapsack can carry with capacity $i$ and considering only the first $k$ items.

Each $\text{opt}(i, k)$ is the value of the optimal solution at a given state $(i, k)$, where the two dimensions of this state represent:

- $i$ — the current **capacity** of the knapsack (the maximum weight used), and
- $k$ — the index of the last (largest) **item** in the optimal solution.

The solution to the original problem is the subproblem $\operatorname{opt}(C, n)$.

- We will solve the problem for each capacity up to $C$ for  $0 \leq i \leq C$.
- At each capacity $i$ we solve the subproblem for each item up to $k$ for $1 \leq k \leq n$.
- Altogether, for each capacity $0 \ldots C$, we will find the optimal solution using items $1 \ldots k$. 

#### Step 3: Deriving a recurrence

For each capacity $i$, the subproblem either includes or does not include item $k$.
...
![[0-1_knapsack_dptable.png|500]]

To compute the value at $(i,k)$ (red), you need the earlier values (blue) to be already computed:
- $\operatorname{opt}(i,k-1)$ — the best value when the $k^{\text{th}}$ item is *not included* 
- $\operatorname{opt}(i-w_k,k-1)$ — the best value when the $k^{\text{th}}$ item *is included*
