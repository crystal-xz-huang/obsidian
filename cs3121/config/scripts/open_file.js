...
function openFileInNewPane(TFile) {
  ...
  console.log("activeLeaf === prevActiveLeaf:", currActiveLeaf === prevActiveLeaf);
}


async function promptOpenFile(tp) {
  const ignorePattern = /^\s*[^a-zA-Z0-9]/;
  const ignoreFolders = ['attachments', 'bases', 'templates', 'config', 'files', 'excalidraw'];
  const ignoreFile = (file) => {
    ...
    if (ignorePattern.test(parentName) || ignorePattern.test(lowerPath)) return true;
