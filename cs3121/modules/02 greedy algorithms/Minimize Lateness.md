...
Similar to [[Activity Selection|Interval Scheduling]]. Except instead of start and finish times, each request $i$ has:
...
- **\[Shortest durationfirst\]** Consider jobs in ascending order of duration $t_j$ (least work first)
...
## Problem: Minimising Job Lateness
...
> [!bookmark] Definition
> Given a schedule S, an **inversion** is a pair of jobs $i$ and $j$ such that $\text{due}_i > \text{due}_j$ but $i$ is scheduled before $j$. (i.e. two jobs are "out of order" relative to their deadlines)
...
![[wk2-minimise-job-lateness-1.png]]
...
![[wk2-minimise-job-lateness-inversions.excalidraw.png]]
